...
docker tag `docker images -q --filter reference=gateway:1.0.0` registry.cn-beijing.aliyuncs.com/winteree/gateway:1.0.0 && \
docker push registry.cn-beijing.aliyuncs.com/winteree/gateway:1.0.0 && \
